...
- (void)initAry {
    _files =  [[NSFileManager defaultManager] contentsOfDirectoryAtPath:AudioPath error:nil];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ...
        cell.textLabel.text = @"alert_message";
    }else {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {           // 振动
        [self playSystemVibrate];
    }else if (indexPath.section == 1) {     // 用户音频
        [self playSoundWithResourceName:@"alert_message" type:@"wav"];
    }else {                                 // 系统音频
        [self playSystemSoundWithName:_files[indexPath.row]];
    }
}
